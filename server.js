...
const startServer = async () => {
  ...
  // DB hygiene: patch missing HB gauge premiums into the active BaseRate document
  try {
    const { BaseRate } = require("./src/models");
    const { DEFAULT_HB_GAUGE_PREMIUMS } = require("./src/services/pricingService");
    const activeRate = await BaseRate.findOne({ isActive: true });
    if (activeRate) {
      const existing = activeRate.hbGaugePremiums || {};
      const merged = { ...DEFAULT_HB_GAUGE_PREMIUMS, ...existing };
      await BaseRate.findByIdAndUpdate(activeRate._id, { hbGaugePremiums: merged });
      logger.info("[BOOT] BaseRate gauge premiums synced");
    }
  } catch (err) {
